...
class Subscription(models.Model):
    ...
    interval_number = fields.Integer(string='Recurrence', default=1)
    interval_type = fields.Selection([('days', 'Days'), ('weeks', 'Weeks'), ('months', 'Months')], string='Recurrence Unit', default='months')
    exec_init = fields.Integer(string='Number of Recurrence')
    date_init = fields.Datetime(string='Start Date', default=fields.Datetime.now)
    state = fields.Selection([('draft', 'Draft'), ('running', 'Running'), ('done', 'Done')], string='Status', copy=False, default='draft')
    doc_source = fields.Reference(selection=_get_document_types, string='Recurrence Template', required=True, help="User can choose the source document on which he wants to create documents")
    doc_lines = fields.One2many('subscription.subscription.history', 'subscription_id', string='Recurrence History', readonly=True)
...
class SubscriptionHistory(models.Model):
    ...
    document_id = fields.Reference(selection=_get_document_types, string='Generated Transaction', required=True)

# multi-company aware

class SubscriptionDocument(models.Model):
    _inherit = ["multi.company.abstract", "subscription.document"]
    _name = "subscription.document"
    _description = "Subscription Template (Multi-Company)"


class SubscriptionDocumentFields(models.Model):
    _inherit = ["multi.company.abstract", "subscription.document.fields"]
    _name = "subscription.document.fields"
    _description = "Subscription Template Fields (Multi-Company)"


class Subscription(models.Model):
    _inherit = ["multi.company.abstract", "subscription.subscription"]
    _name = "subscription.subscription"
...
class SubscriptionHistory(models.Model):
    ...
    _name = "subscription.subscription.history"
    _description = "Subscription History (Multi-Company)"
